define(
  ...
  function(dojo, coweb, dijit, DataGrid,
    ...
    proto.init = function() {
      ...
      var switchPageButton = dijit.byId("switchPage");
      ...
      dojo.connect(switchPageButton, "onClick", this, "onSwitch");


      dojo.query(".songInLib").on("click", function(e){
        var date = new Date();
        var id = String(Math.random()).substr(2) + String(date.getTime());
        var toBeAdded = {
          id: id,
          song: "This music add from lib",
          singer: "this artist from lib"
        };
        $(self)[0].dataStore.newItem(toBeAdded);
        console.log("add new song from lib! added: " + toBeAdded.id);
      });
    ...
    proto.onSwitch = function() {
      console.log("about to swich page");
      this.toggleDivs();
    }

    proto.toggleDivs =  function(){
      if(   dojo.style(dojo.byId('lib'), "display") == "none"){
        dojo.style(dojo.byId('lib'), "display", "block");
        dojo.style(dojo.byId('playlist'), "display", "none");
      } else {
        dojo.style(dojo.byId('lib'), "display", "none");
        dojo.style(dojo.byId('playlist'), "display", "block");
      }
    }
